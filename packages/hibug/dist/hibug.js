(function (global, factory) {
  ...
  (global = global || self, global.Hibug = factory());
  ...
  const WEBSOCKET_ERROR = 'websocketError'; // websocket 错误

  const REPORT_INFO = 'reportInfo'; // 主动上报的信息

  const UNKNOWN_ERROR = 'unknownError'; // 未知错误
  ...
  function report(data) {
    try {
      ...
      if (config === null || config === void 0 ? void 0 : config.report) {
  ...
  function handleError(error) {
    ...
    if (errorList.length) {
      ...
      errorList.forEach(function (e) {
        if (!isEqual_1(error.desc, e.desc)) {
          errorList.push(_objectSpread({}, getBaseInfo(), error));
        }
      });
    } else {
      errorList.push(_objectSpread({}, getBaseInfo(), error));
    ...
    if ((config === null || config === void 0 ? void 0 : config.error) && (!(config === null || config === void 0 ? void 0 : config.mode) || config.mode === 'immediately') && errorList.length && errorList.length <= config.maxError) {
  ...
  function caughtError(target, name, descriptor) {
    ...
    if (!((_window = window) === null || _window === void 0 ? void 0 : _window.$HibugAuth)) {
      console.error('Hibug: 检测到未执行 Hibug.init()');
      return;
    }
  ...
  function getHttpRequestError() {
    ...
    var ignore = window.$HibugConfig.ignore;
  ...
  function getPerformance() {
    try {
      ...
      if (!((_window = window) === null || _window === void 0 ? void 0 : _window.performance)) return new Error('performance is not supported');
  ...
  function getUV() {
    try {
      if (window.localStorage) {
        var now = new Date();
        var UV = localStorage.getItem('$HibugUV') || '';
        var time = localStorage.getItem('$HibugUVTime') || '';
  ...
  function _init() {
    var _hibugConfig = window.$HibugConfig;
    /**
    ...
    listener && listener('error', function (e) {
      getError({
        type: 'default',
        e: e
      });
    ...
    listener && listener('unhandledrejection', function (e) {
      e.preventDefault();
      getError({
        type: 'promise',
        e: e
    ...
    if ((_hibugConfig === null || _hibugConfig === void 0 ? void 0 : _hibugConfig.mode) === 'beforeunload') {
      listener && listener('unload', function () {
        if ((errorList === null || errorList === void 0 ? void 0 : errorList.length) && errorList.length <= _hibugConfig.maxError) {
          report(errorList);
        }
      }, true);
    }

    if (_hibugConfig === null || _hibugConfig === void 0 ? void 0 : _hibugConfig.performance) {
      listener && listener('load', function () {
        ...
        if (resource === null || resource === void 0 ? void 0 : resource.length) data.resource = resource;
        if (UV) data.UV = UV;
        report(data);
      }, true);
    }
  ...
  Hibug.init = function (conf) {
    ...
    if (!((_window = window) === null || _window === void 0 ? void 0 : _window.$HibugConfig)) {
      // default config
      window.$HibugConfig = {
        delay: 2000,
        // 错误处理间隔时间
        ...
        // 最大上传错误数量
        mode: 'immediately',
        // 信息发送模式 immediately 立即发送 beforeunload 页面注销前发送
        ignore: [],
    ...
    if (conf) {
      window.$HibugConfig = _objectSpread({}, window.$HibugConfig, conf);
    }

    if ((_window2 = window) === null || _window2 === void 0 ? void 0 : _window2.$HibugAuth) _init();
  };
